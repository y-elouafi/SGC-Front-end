<template lang="html">
  ...
  <section class="create-commande  pl-2 pr-2 pt-2">
    <h1 class="alert alert-success br">Creation Commande Client</h1>
    <div class="">
        ...
        await setTimeout(() => {
          for(var i=0; i<this.cmd.ids.length-1 ; i++)
          {
            ...
            while(this.cmd.ids[index] == this.cmd.ids[i])
            {
              ...
              // this.cmd.to_delete.splice(index, 1);
              // this.cmd.ids.splice(index, 1);
              // this.cmd.noms.splice(index, 1);
              // this.cmd.cat_noms.splice(index, 1);
              // this.cmd.stockes.splice(index, 1);
              // this.cmd.qtes.splice(index, 1);
              // this.cmd.achats.splice(index, 1);
              // this.cmd.vents.splice(index, 1);
              // this.cmd.last_vents.splice(index, 1);
              // this.cmd.totals.splice(index, 1);
              // this.calculeTotalIndex();
              return false;
            }
          }
      ...
      async saveCommande(){
        ...
        if(this.cmd.ids.length > 0 )
        {
          if(await this.verifyDeplicateArticleBeforSave()){
            axios.post('/client_commande/create_new_commande_client',
              {
                ...
                if(data)
                {
                  alert("Votre Commande N "+ data.id +" a ete bien traite!");
                  this.videArrayCommande();
                }
